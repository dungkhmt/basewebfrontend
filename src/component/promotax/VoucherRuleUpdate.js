...
const TYPE_LIST = [{code: 'APPLY', text: 'Áp dụng'}, {code: 'DO_NOT_APPLY', text: 'Không áp dụng'}];
...
export function VoucherRuleUpdate() {
  ...
  const [productTransportCategoryId, setProductTransportCategoryId] = useState('');
  ...
  async function getVoucherRule() {
    let voucherRule = await authGet(dispatch, token, `/get-voucher-rule/${voucherRuleId}`);
    if (voucherRule) {
  ...
  async function handleSubmit() {
    ...
    let voucherRule = await authPost(dispatch, token, `/update-voucher-rule/${voucherRuleId}`, body)
      .then(r => r.json());
  ...
  return (<div>
    ...
    {textField('productCategoryId', 'Nhóm sản phẩm áp dụng', 'search', productCategoryId, setProductCategoryId)}
    {textField(
      'productTransportCategoryId',
    ...
    {textField('vendorCode', 'Mã nhà cung cấp áp dụng', 'search', vendorCode, setVendorCode)}
    {textField('vendorCategoryId', 'Nhóm nhà cung cấp áp dụng', 'search', vendorCategoryId, setVendorCategoryId)}
    {textField('customerCode', 'Mã khách hàng áp dụng', 'search', customerCode, setCustomerCode)}
    {textField('customerCategoryId', 'Nhóm khách hàng áp dụng', 'search', customerCategoryId, setCustomerCategoryId)}
    {textField('paymentMethod', 'Phương thức thanh toán áp dụng', 'search', paymentMethod, setPaymentMethod)}

    <Button color={'primary'} variant={'contained'} onClick={() => handleSubmit()}>Cập nhật quy tắc</Button>
  </div>);
}
