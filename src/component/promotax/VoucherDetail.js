...
export function VoucherDetail() {
  ...
  const [voucherRuleColumns,] = useState(
    [
      {field: 'type', title: 'Loại', render: rowData => rowData['type'] === 'APPLY' ? 'Áp dụng' : 'Không áp dụng'},
      ...
      {field: 'createdDate', title: 'Ngày tạo'},
      {
        title: '', render: rowData => (<div>
          <Button variant={"contained"} color={"primary"} startIcon={<EditIcon/>}
                  onClick={() => history.push(`/promo-group/edit-voucher-rule/${voucherId}/${rowData['voucherRuleId']}`)}/>
          <Button variant={"contained"} color={"primary"} startIcon={<DeleteIcon/>}
                  onClick={() => {
                    if (window.confirm(`Xác nhận xóa?`)) {
                      authGet(dispatch, token, `/delete-voucher-rule/${rowData['voucherRuleId']}`).then(r => {
  ...
  return (<div>
      ...
      <tr>
        <td style={{width: '50%'}}>
          <table style={{width: '100%', border: '0px'}}>
            ...
            <tr>
              <td>Mô tả:</td>
              <td><b>{voucher['description']}</b></td>
            </tr>
            <tr>
              <td>Ngày hiệu lực:</td>
              <td><b>{voucher['fromDate'] ? dateFnFormat(new Date(voucher['fromDate']), 'yyyy-MM-dd') : ''}</b></td>
            </tr>
            <tr>
              <td>Ngày kết thúc:</td>
              <td><b>{voucher['thruDate'] ? dateFnFormat(new Date(voucher['thruDate']), 'yyyy-MM-dd') : ''}</b></td>
            </tr>
            <tr>
              <td>Ngày tạo:</td>
              <td><b>{voucher['createdDate'] ? dateFnFormat(new Date(voucher['createdDate']), 'yyyy-MM-dd') : ''}</b>
              </td>
            </tr>
            <tr>
              <td>Giá trị giảm tối thiểu:</td>
              <td><b>{currencyFormat(voucher['minDiscountAmount'])}</b></td>
            </tr>
            <tr>
              <td>Giá trị giảm tối đa:</td>
              <td><b>{currencyFormat(voucher['maxDiscountAmount'])}</b></td>
            </tr>
          </table>
        </td>
        <td style={{width: '50%'}}>
          <table style={{width: '100%', border: '0px'}}>
            ...
            <tr>
              ...
              <td><b>{voucher['minDiscountRate']}</b></td>
            </tr>
            <tr>
              <td>Tỉ lệ giảm tối đa:</td>
              <td><b>{voucher['maxDiscountRate']}</b></td>
            </tr>
            <tr>
              <td>Đơn tối thiểu:</td>
              <td><b>{currencyFormat(voucher['minOrderValue'])}</b></td>
            </tr>
            <tr>
              <td>Số lần sử dụng:</td>
              <td><b>{voucher['usageLimit']}</b></td>
            </tr>
            <tr>
              <td>Số lần sử dụng từng khách:</td>
              <td><b>{voucher['usageLimitPerCustomer']}</b></td>
            </tr>
            <tr>
              <td>Số lần đã sử dụng:</td>
              <td><b>{voucher['usageCount']}</b></td>
            </tr>
          </table>
    ...
    <div>
      <MaterialTable columns={voucherRuleColumns} data={query => getVoucherRules(query.page, query.pageSize)}
                     title={'Danh sách các quy tắc cho mã giảm giá'} options={{search: false}}/>
    </div>
  </div>);
}
