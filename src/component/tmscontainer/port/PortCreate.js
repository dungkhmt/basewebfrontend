...
function PortCreate(props) {
  ...
  const handlePortIdChange = (event) => {
  ...
  const handleSubmit = (event) => {
    ...
    authPost(dispatch, token, "/create-port", data).then(
      (res) => {
        console.log(res);
        setIsRequesting(false);
        if (res.status === 401) {
    ...
    <div>
      <Grid container spacing={5}>
        <Grid item xs={5}>
          ...
          <Typography variant="h6">Mã bến cảng: </Typography>
          ...
          <br />
          <br />
          <Typography variant="h6">Tên bến cảng: </Typography>
