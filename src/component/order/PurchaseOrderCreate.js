...
export default function PurchaseOrderCreate() {
  ...
  async function getSupplier() {
    let supplier = await authGet(
      dispatch,
      token,
      "/get-supplier-by-id/" + supplierPartyId
    );
  ...
  async function getUnitPrices() {
    let productPriceSuppliers = await authGet(
      dispatch,
      token,
    ...
    productPriceSuppliers.forEach(
      (productPriceSupplier) =>
        (unitPrices[productPriceSupplier["productId"]] =
          productPriceSupplier["unitPrice"])
    );
  ...
  useEffect(() => {
    Promise.all([getSupplier(), getProductList(), getUnitPrices()]).then(
      (r) => r
    );
  }, []);
  ...
  return (
    <div>
      ...
      <Button
        color={"secondary"}
        variant={"contained"}
        onClick={() => history.push("/purchase-order/list")}
      >
        Hủy
      </Button>
    </div>
