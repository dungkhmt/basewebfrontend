...
const postmanColumns = [
  ...
  {
    title: "Tên bưu cục",
    field: "postOffice.postOfficeName",
    editable: false,
  },
];
const postDriverColumns = [
  ...
  { title: "Họ tên", field: "postDriverName" },
  {
    ...
    filtering: false,
    render: (postDriver) => (
      <Link to={"/postoffice/post-driver-detail/" + postDriver.postDriverId}>
...
export default function ManagePostman(props) {
  ...
  const loadData = async () => {
    await Promise.all([
      authGet(dispatch, token, "/get-postman-list/", {}).then((response) => {
        setPostmanData(response);
      }),
      authGet(dispatch, token, "/get-post-driver-list/", {}).then(
        (response) => {
