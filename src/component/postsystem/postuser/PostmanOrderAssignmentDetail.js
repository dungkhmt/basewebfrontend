    ...
    if (elements.type == Marker) {
      ...
      bounds.extend(
        new window.google.maps.LatLng(
          elements.props.position.lat,
          elements.props.position.lng
        )
      );
      return;
    } else {
...
function PostmanOrderAssignmentDetail(props) {
  ...
  const loadData = async (fromDate, toDate) => {
    await Promise.all([
      authGet(
        dispatch,
        token,
        "/get-order-by-postman-and-date?fromDate=" +
          formatDate(fromDate) +
          "&toDate=" +
          formatDate(toDate),
        {}
      ).then((res) => {
        authGet(dispatch, token, "/get-post-office-by-postman", {}).then(
          (res1) => {
            setData(res);
            setPostOffice(res1);
            setPickTableData(
              [...res.pickAssignment, ...res.finishedAssignment].filter(
                (assignment) =>
                  assignment.statusId ==
                    "POST_ORDER_ASSIGNMENT_PICKUP_WAITING" ||
                  assignment.statusId == "POST_ORDER_ASSIGNMENT_PICKUP_SUCCESS"
              )
            );
            setShipTableData(
              [...res.shipAssignment, ...res.finishedAssignment].filter(
                (assignment) =>
                  assignment.statusId == "POST_ORDER_ASSIGNMENT_SHIP_WAITING" ||
                  assignment.statusId == "POST_ORDER_ASSIGNMENT_SHIP_SUCCESS"
              )
            );
          }
        );
  ...
  const tspSolve = () => {
    ...
    authPost(
      dispatch,
      token,
      "/solve-postman-post-order-assignment-tsp?pick=" +
        pick +
        "&postOfficeId=" +
        postOffice.postOfficeId,
      {
        postShipOrderPostmanLastMileAssignmentIds:
          tabValue == 0
            ? data.pickAssignment.map(
                (x) => x.postShipOrderPostmanLastMileAssignmentId
              )
            : data.shipAssignment.map(
                (x) => x.postShipOrderPostmanLastMileAssignmentId
              ),
      ...
      .then((res) => {
        ...
        if (res.length > 0) {
          ...
          res.forEach((assignment) => {
            geoPoints.push(
              assignment.postOrder.fromCustomer.postalAddress.geoPoint
            );
          });
          geoPoints.push(postOffice.postalAddress.geoPoint);
  ...
  useEffect(() => {
    ...
    if (map.props.children && map.props.children.length > 0)
  ...
  const orderUpdate = (newData, oldData, pick) => {
    authPost(dispatch, token, "/update-postman-post-order-assignment", {
      postShipOrderPostmanLastMileAssignmentId:
        oldData.postShipOrderPostmanLastMileAssignmentId,
  ...
  return (
    <MuiPickersUtilsProvider utils={DateFnsUtils}>
        ...
        <Grid item xs={7}>
          ...
          >
            ...
            >
              ...
              {tabValue == 0
                ? data.pickAssignment.map((assignment) => {
                ...
                : undefined}
              {tabValue == 1
                ? data.shipAssignment.map((assignment) => {
                    let order = assignment.postOrder;
                    return (
                      <Marker
              ...
              {data.finishedAssignment.map((assignment) => {
                ...
                if (
                  assignment.statusId ==
                    "POST_ORDER_ASSIGNMENT_PICKUP_SUCCESS" &&
                  tabValue == 0
                )
                  return (
                    <Marker
                    ...
                    return (
                      <Polyline
                        path={[
                          ...
                          {
                            lat: pickGeoPoints[i + 1].latitude,
                            lng: pickGeoPoints[i + 1].longitude,
                          },
                        ]}
                        icons={[
                          {
                    ...
                    return (
                      <Polyline
                        path={[
                          { lat: geoPoint.latitude, lng: geoPoint.longitude },
                          {
                            lat: shipGeoPoints[i + 1].latitude,
                            lng: shipGeoPoints[i + 1].longitude,
                          },
                        ]}
                        icons={[
                          {
      ...
      <Button
        ...
        variant="outlined"
        disabled={
          !(tabValue == 0 && data.pickAssignment.length > 0) ||
          (tabValue == 1 && data.shipAssignment.length > 0)
        }
        onClick={() => tspSolve()}
