...
function PostOrderDetail(props) {
  ...
  const loadData = (postShipOrderId, fromPostOffice, toPostOffice) => {
    authGet(
      dispatch,
      token,
      "/get_post_order_route?postOrderId=" + postShipOrderId
    )
      .then((response) => {
        setRoutes(response);
        let postOffices = [];
      ...
      <Grid container spacing={5} xs={12}>
        <Grid item xs={5}>
          ...
          <Paper className={classes.root}>
            ...
            <TextField id="weight" value={weight} fullWidth label="Khối lượng">
              <br />
            </TextField>
            <TextField
              id="description"
            ...
            <TextField id="status" value={status} fullWidth label="Trạng thái">
              <br />
            </TextField>
            <TextField
              id="fromPostOffice"
              value={
                fromPostOffice ? fromPostOffice.postOfficeName : "Chưa xác định"
              }
              fullWidth
            ...
            <TextField
              id="toPostOffice"
              value={
                toPostOffice ? toPostOffice.postOfficeName : "Chưa xác định"
              }
              fullWidth
              label="Bưu cục đích"
            >
              ...
              {fromPostOffice ? (
                <Polyline
                  path={[
                    ...
                    {
                      lat: parseFloat(
                        fromPostOffice.postalAddress.geoPoint.latitude
                      ),
                      lng: parseFloat(
                        fromPostOffice.postalAddress.geoPoint.longitude
                      ),
                    },
                  ]}
                  icons={[
              ...
              {toPostOffice ? (
                <Polyline
                  path={[
                    {
                      lat: parseFloat(
                        toPostOffice.postalAddress.geoPoint.latitude
                      ),
                      lng: parseFloat(
                        toPostOffice.postalAddress.geoPoint.longitude
                      ),
                    },
                    { lat: toLat, lng: toLng },
                  ]}
              ...
              {routes.map((route) => {
                return (
                  <Polyline
                    path={[
                      {
                        lat: route.fromPostOffice.postalAddress.geoPoint
                          .latitude,
                        lng: route.fromPostOffice.postalAddress.geoPoint
                          .longitude,
                      },
                      {
                        lat: route.toPostOffice.postalAddress.geoPoint.latitude,
                        lng: route.toPostOffice.postalAddress.geoPoint
                          .longitude,
                      },
                    ]}
                    icons={[
