...
export default function ExecuteTrip() {
  ...
  const taskListColumn = [
    {
      ...
      customSort: (a, b) => {
        return a.havingOrder - b.havingOrder;
      },
      defaultSort: "desc",
    },
    {
      title: "Xuất phát",
      field: "postOfficeTrip.fromPostOffice.postOfficeName",
    },
    {
      title: "Điểm đến",
      field: "postOfficeTrip.toPostOffice.postOfficeName",
    },
    {
      title: "Giờ chạy",
      field: "scheduleDepartureTime",
    },
  ];
  ...
  const handleOnSelectionChange = (rows) => {
    setActiveAssignments(
      orderTableRef.state.data.filter((x) => x.tableData.checked)
    );
  };
  const getIcon = (row) => {
    switch (row.status) {
  ...
  const getNotificationIcon = (row) => {
    if (!row.havingOrder)
      return (
        <Tooltip title={"Không có đơn hàng"}>
          <NotificationsIcon disabled />
        </Tooltip>
      );
    return (
      <Tooltip title={"Có đơn hàng cần xử lý"}>
  ...
  const onClickNotificationActions = (row) => {
    ...
    newPostTrip.assignment = [
      ...newPostTrip.assignment,
      ...availableAssignment,
    ];
    newPostTrip.assignment.forEach((assignment) => {
      if (assignment.tableData) assignment.tableData.checked = false;
    });
            ...
            authDelete(dispatch, token, "/delete-execute-trip", {
              ...
              if (res.status == "SUCCESS") {
                ...
                newData.forEach((fixedtrip) => {
                  if (
                    fixedtrip.postOfficeFixedTripExecuteId ==
                    row1.postOfficeFixedTripExecuteId
                  ) {
                    newAvailableAssignment = [
                      ...availableAssignment,
                    ...
                    newAvailableAssignment.forEach((assignment) => {
                      if (
                        assignment.postOfficeTrip.postOfficeTripId ==
                        fixedtrip.postOfficeTrip.postOfficeTripId
                      ) {
        ...
        setConfirmAction({
          ...
          handleSuccess: () => {
            authPost(dispatch, token, "/execute-trip", {
              ...
              status: "WAITING",
              postShipOrderTripPostOfficeAssignmentIds: row1.assignment.map(
                (assignment) =>
                  assignment.postShipOrderPostOfficeTripAssignmentId
              ),
            }).then((res) => {
              let newFixedTripExecutes = copyObj(fixedTripExecutes);
              newFixedTripExecutes[row1.postOfficeFixedTripId] =
                res.postOfficeFixedTripExecuteId;
              setFixedTripExecutes(newFixedTripExecutes);
              row1.status = "WAITING";
              row1.postOfficeFixedTripExecuteId =
                res.postOfficeFixedTripExecuteId;
  ...
  const handleRemoveAssignmnet = (assignment) => {
    ...
    newData.forEach((fixedtrip) => {
      fixedtrip.assignment = fixedtrip.assignment.filter((x) => {
        return (
          x.postShipOrderPostOfficeTripAssignmentId !=
          assignment.postShipOrderPostOfficeTripAssignmentId
        );
      });
      newAvailableAssignment.forEach((assignment) => {
  ...
  const handleAddAssignmnet = () => {
    let activeAssignmentIds = activeAssignments.map(
      (x) => x.postShipOrderPostOfficeTripAssignmentId
    );
    let newAvailableAssignment = availableAssignment.filter((assignment) => {
      return !activeAssignmentIds.includes(
        assignment.postShipOrderPostOfficeTripAssignmentId
      );
    });
    setAvailableAssignment(newAvailableAssignment);
    ...
    newData.forEach((fixedtrip) => {
      activeAssignments.forEach((assignment) => {
        if (
          fixedtrip.postOfficeTrip.postOfficeTripId ==
            assignment.postOfficeTrip.postOfficeTripId &&
          fixedtrip.postOfficeFixedTripId == activeTrip.postOfficeFixedTripId
        ) {
  ...
  const orderColumn = [
    {
      title: "Mã đơn hàng",
      field: "postOrder.postShipOrderId",
      sorting: false,
    },
    {
      title: "Người gửi",
      field: "postOrder.fromCustomer.postCustomerName",
      sorting: false,
    },
    {
      title: "Người nhận",
      field: "postOrder.toCustomer.postCustomerName",
      sorting: false,
    },
    {
      title: "Số điện thoại người gửi",
      field: "postOrder.fromCustomer.phoneNum",
      sorting: false,
    },
    {
      title: "Số điện thoại người nhận",
      field: "postOrder.toCustomer.phoneNum",
    },
    {
      title: "Địa chỉ Người gửi",
      field: "postOrder.fromCustomer.postalAddress.address",
      sorting: false,
    },
    {
      title: "Địa chỉ người nhận",
      field: "postOrder.toCustomer.postalAddress.address",
      sorting: false,
    },
    {
      title: "",
      ...
      render: (assignment) => {
        return (
          <IconButton
            color="primary"
            onClick={(e) => {
              handleRemoveAssignmnet(assignment);
            }}
        ...
        authGet(
          ...
          .then((res) => {
            ...
            newData.forEach((fixedtrip) => {
              ...
              res.forEach((assignment) => {
                if (
                  assignment.postOfficeTrip.fromPostOfficeId ==
                    fixedtrip.postOfficeTrip.fromPostOfficeId &&
                  assignment.postOfficeTrip.toPostOfficeId ==
                    fixedtrip.postOfficeTrip.toPostOfficeId
                ) {
                  fixedtrip.havingOrder = true;
                }
              });
          ...
          .then((newData) => {
            authGet(
              dispatch,
              token,
              "/get-post-execute-trip-list?date=" + formatDate(chooseDate),
              {}
            )
              .then((res) => {
                let newFixedTripExecutes = copyObj(fixedTripExecutes);
                res.forEach((fixedTripExecute) => {
                  newData.forEach((fixedtrip) => {
                    if (
                      fixedtrip.postOfficeFixedTripId ==
                      fixedTripExecute.postOfficeFixedTripId
                    ) {
                      fixedtrip.assignment = [
                        ...fixedtrip.assignment,
                        ...fixedTripExecute.postShipOrderFixedTripPostOfficeAssignments.map(
                          (postShipOrderFixedTripPostOfficeAssignment) => {
                            let assignment =
                              postShipOrderFixedTripPostOfficeAssignment.postShipOrderTripPostOfficeAssignment;
                            assignment.choosen = true;
                            return assignment;
                          }
                        ),
                      ...
                      fixedtrip.postOfficeFixedTripExecuteId =
                        fixedTripExecute.postOfficeFixedTripExecuteId;
                    }
                  });
                });
                setData(newData);
    ...
    authGet(
      ...
      .then((newData) => {
        authGet(
          dispatch,
          token,
          "/get-post-execute-trip-list?date=" + formatDate(newChooseDate),
          {}
        )
          .then((res) => {
          ...
          actions={[
            ...
            (postTrip) => ({
              ...
              onClick: (event, postOrder) =>
                onClickNotificationActions(postOrder),
