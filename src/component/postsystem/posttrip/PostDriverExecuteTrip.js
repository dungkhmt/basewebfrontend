...
export default function PostDriverExecuteTrip() {
  ...
  useEffect(() => {
    authGet(dispatch, token, "/my-account", {})
      .then((response) => setPostDriverId(response.partyId))
      .then(() =>
        authGet(dispatch, token, "/get-post-trip-list-by-post-driver", {}).then(
          (response) => {
            setData(response);
            authGet(
              dispatch,
              ...
              .then((newData) => {
                authGet(
                  dispatch,
                  token,
                  "/get-post-execute-trip-list-by-post-driver?date=" +
                    formatDate(chooseDate),
                  {}
                )
                  .then((res) => {
            ...
            res.forEach((fixedTripExecute) => {
              newData.forEach((fixedtrip) => {
                if (
                  ...
                  newFixedTripExecutes[fixedtrip.postOfficeFixedTripId] =
                    fixedTripExecute.postOfficeFixedTripExecuteId;
                  fixedtrip.assignment = [
                    ...fixedtrip.assignment,
                    ...fixedTripExecute.postShipOrderFixedTripPostOfficeAssignments.map(
