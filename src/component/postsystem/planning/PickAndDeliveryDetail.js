...
function PickAndDeliveryDetail(props) {
  ...
  const geoPointsGen = (postman) => {
    ...
    if (postman.postOrders.length > 0) {
      ...
      postman.postOrders.forEach((postOrder) => {
        postman.distance += distance(
          postOrder.fromCustomer.postalAddress.geoPoint,
          postman.geoPoints[postman.geoPoints.length - 1]
        );
        postman.geoPoints.push(postOrder.fromCustomer.postalAddress.geoPoint);
      });
      postman.distance += distance(
        postOffice.postalAddress.geoPoint,
        postman.geoPoints[postman.geoPoints.length - 1]
      );
      postman.geoPoints.push(postOffice.postalAddress.geoPoint);
    }
    ...
    authPost(dispatch, token, "/post-office-vrp-solve", {
      ...
      .then((res) => {
        if (res.solutionFound) {
          ...
          setSolved(true);
          setFromPostOrder(
            fromPostOrder.filter(
              (postOrder) =>
                !solvedPostOrderId.includes(postOrder.postShipOrderId)
            )
          );
        }
        setSolving(false);
  ...
  const submitSolution = () => {
    let requestbody = postmen.map((postman) => {
      return {
        ...
        postOrderIds: postman.postOrders.map((postOrder) => {
    ...
    authPost(
      dispatch,
      token,
      "/submit-postman-assign/?pick=" + true,
      requestbody
    )
      .then((res) => {
        ...
        authGet(
          ...
          res1.forEach((postman) => {
            ...
            if (postman.postOrders.length > 0) {
              ...
              postman.distance += distance(
                res.postOffice.postalAddress.geoPoint,
                postman.geoPoints[postman.geoPoints.length - 1]
              );
              postman.geoPoints.push(res.postOffice.postalAddress.geoPoint);
            }
  ...
  const taskListColumn = [
    ...
    },
    {
      ...
      render: (postman) => {
        return (
          Math.round((postman.distance ? postman.distance : 0) / 1000) + " km"
        );
      },
    },
    {
      ...
      render: (postman) => {
        return (
          <IconButton
            color="primary"
            onClick={(e) => {
              handleViewRouteChange(postman.tableData.id, !postman.viewRoute);
            }}
  ...
  const orderColumn = [
    ...
    {
      title: "Người gửi",
      field: "fromCustomer.postCustomerName",
      sorting: false,
    },
    {
      title: "Người nhận",
      field: "toCustomer.postCustomerName",
      sorting: false,
    },
    {
      title: "Số điện thoại người gửi",
      field: "fromCustomer.phoneNum",
      sorting: false,
    },
    { title: "Số điện thoại người nhận", field: "toCustomer.phoneNum" },
    {
      title: "Địa chỉ Người gửi",
      field: "fromCustomer.postalAddress.address",
      sorting: false,
    },
    {
      title: "Địa chỉ người nhận",
      field: "toCustomer.postalAddress.address",
      sorting: false,
    },
    {
      title: "Chuyển cho",
      ...
      render: (postOrder) => {
        return (
          <Autocomplete
            id="combo-box-demo"
            options={postmen.filter(
              (postman) => postman.postmanId != activePostman.postmanId
            )}
            getOptionLabel={(option) => option.postmanName}
            style={{ width: 200 }}
            renderInput={(params) => (
              <TextField {...params} label="Chuyển cho" variant="outlined" />
            )}
            onChange={(event, value) =>
              moveToPostman(value.postmanId, postOrder.postShipOrderId)
            }
          />
        );
      },
      ...
      render: (postOrder) => {
        return (
          <Autocomplete
            ...
            onChange={(event, value) =>
              moveToPostman(
                value ? value.postmanId : undefined,
                postOrder.postShipOrderId
              )
            }
          />
        );
  ...
  const deleteOrder = (postOrder) => {
    ...
    newPostmen.forEach((_postman) => {
      if (_postman.postmanId == activePostman.postmanId) {
        _postman.postOrders = _postman.postOrders.filter(
          (_postOrder) =>
            _postOrder.postShipOrderId != postOrder.postShipOrderId
        );
  ...
  const moveToPostman = (toPostmanId, postOrderId) => {
    let newPostmen = JSON.parse(JSON.stringify(postmen));
    let newActivePostman = activePostman
      ? JSON.parse(JSON.stringify(activePostman))
      : activePostman;
    if (activePostman) {
      newPostmen.forEach((_postmanSource) => {
        if (_postmanSource.postmanId == activePostman.postmanId) {
          ...
          let newPostOrders = _postmanSource.postOrders.filter(
            (postOrder) => postOrder.postShipOrderId != postOrderId
          );
        ...
        if (_postmanTarget.postmanId == toPostmanId) {
          fromPostOrder.forEach((postOrder) => {
            if (postOrder.postShipOrderId == postOrderId) {
              ...
              let newFromPostOrder = fromPostOrder.filter(
                (postOrder) => postOrder.postShipOrderId != postOrderId
              );
  ...
  const handleOnSelectionChange = (rows) => {
    if (rows) {
      setSolvingPostmen(
        postmanTableRef.state.data
          .filter((x) => x.tableData.checked)
          .map((postman) => postman.postmanId)
      );
    }
  };
    ...
    <MuiPickersUtilsProvider utils={DateFnsUtils}>
      <Grid container spacing={5}>
        <Grid item xs={5}>
          ...
          />
          <br />
          <Button color="primary" variant="outlined" onClick={submitSolver}>
                ...
                ? fromPostOrder.map((order) => {
                    return (
                      <Marker
                        ...
                        position={{
                          lat: order.fromCustomer.postalAddress.geoPoint
                            .latitude,
                          lng: order.fromCustomer.postalAddress.geoPoint
                            .longitude,
                        }}
                    ...
                    return postman.geoPoints[i + 1] ? (
                      <Polyline
                        path={[
                          ...
                          {
                            lat: postman.geoPoints[i + 1].latitude,
                            lng: postman.geoPoints[i + 1].longitude,
                          },
                        ]}
                        icons={[
                          {
      ...
      <Dialog open={open} onClose={handleCancelDialog} fullWidth maxWidth>
        <DialogTitle>
          {"Danh sách đơn hàng của " +
            (activePostman ? activePostman.postmanName : null)}
        </DialogTitle>
        <DialogContent>
          <MaterialTable
            className={classes.table}
            ...
            actions={[
              (postOrder) => ({
                ...
                onClick: (event, postOrder) =>
                  moveOrder(activePostman, postOrder.order, -1),
              ...
              (postOrder) => ({
                ...
                onClick: (event, postOrder) =>
                  moveOrder(activePostman, postOrder.order, 1),
                disabled:
                  postOrder.tableData.id == activePostman.postOrders.length - 1,
              }),
              (postOrder) => ({
                icon: "delete",
                tooltip: "Xóa",
