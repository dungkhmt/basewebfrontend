...
function ShipAndDeliveryDetail(props) {
    ...
    if (postman.postOrders.length > 0) {
      ...
      postman.postOrders.forEach((postOrder) => {
        postman.distance += distance(
          postOrder.toCustomer.postalAddress.geoPoint,
  ...
  const submitSolution = () => {
    ...
    authPost(
      dispatch,
      token,
      "/submit-postman-assign?pick=" + false,
      requestbody
    )
      .then((res) => {
        ...
        if (_postmanTarget.postmanId == toPostmanId) {
          toPostOrder.forEach((postOrder) => {
            if (postOrder.postShipOrderId == postOrderId) {
              ...
              let newToPostOrder = toPostOrder.filter(
                (postOrder) => postOrder.postShipOrderId != postOrderId
              );
                    ...
                    return (
                      <Marker
                        ...
                        position={{
                          ...
                          lng: order.toCustomer.postalAddress.geoPoint
                            .longitude,
                        }}
                    ...
                    return (
                      <Polyline
                        path={[
                          ...
                          {
                            lat: order.toCustomer.postalAddress.geoPoint
                              .latitude,
                            lng: order.toCustomer.postalAddress.geoPoint
                              .longitude,
                          },
