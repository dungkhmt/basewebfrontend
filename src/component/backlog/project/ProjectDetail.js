...
export default function ProjectDetail(props) {
  ...
  const [openSuggestionResultDialog, setOpenSuggestionResultDialog] =
    useState(false);
  const [suggestData, setSuggestData] = useState([]);
  const [suggestChartData, setSuggestChartData] = useState({});
  const [openSuggestAlert, setOpenSuggestAlert] = useState(false);
  const [openSuggestInstructionAlert, setOpenSuggestInstructionAlert] =
    useState(false);
  const [currentTableData, setCurrentTableData] = useState();
  const [tablePageSize, setTablePageSize] = useState(5);

  const [addMemberDialogOpen, setAddMemberDialogOpen] = useState(false);
  const [showMemberListDialogOpen, setShowMemberListDialogOpen] =
    useState(false);
  ...
  function normalizeProjectDetail(data, account) {
    ...
    data.forEach((task) => {
      ...
      if (
        task.backlogTask.attachmentPaths != null &&
        task.backlogTask.attachmentPaths !== undefined &&
        task.backlogTask.attachmentPaths.length > 0
      )
        task.backlogTask.attachmentPaths =
          task.backlogTask.attachmentPaths.split(";");
      else task.backlogTask.attachmentPaths = [];

      task.backlogTask.statusName = TASK_STATUS.LIST.filter(
        (status) => status.statusId === task.backlogTask.statusId
      ).map((e) => e.description)[0];
      task.backlogTask.priorityName = TASK_PRIORITY.LIST.filter(
        (priority) => priority.priorityId === task.backlogTask.priorityId
      ).map((e) => e.priorityName)[0];
      task.backlogTask.categoryName = TASK_CATEGORY.LIST.filter(
        (category) => category.categoryId === task.backlogTask.categoryId
      ).map((e) => e.categoryName)[0];
      ...
      if (selectedRows.includes(task.backlogTask.backlogTaskId))
        task.tableData = { checked: true };
    });
  ...
  const [taskListColumn, setTaskListColumn] = useState([
    ...
    {
      title: "Loại",
      field: "backlogTask.categoryName",
      // defaultSort: 'asc',
      render: (rowData) => {
        const color = categoryPool
          .filter((x) => x.categoryId === rowData.backlogTask.categoryId)
          .map((e) => e.color);
    ...
    {
      title: "Trạng thái",
      field: "backlogTask.statusName",
      // defaultSort: 'asc',
      render: (rowData) => {
        const color = statusPool
          .filter((x) => x.statusId === rowData.backlogTask.statusId)
          .map((e) => e.color);
    ...
    {
      title: "Độ ưu tiên",
      field: "backlogTask.priorityName",
      filtering: false,
      // defaultSort: 'asc',
      render: (rowData) => {
        const color = priorityPool
          .filter((x) => x.priorityId === rowData.backlogTask.priorityId)
          .map((e) => e.color);
        const icon = priorityPool
          .filter((x) => x.priorityId === rowData.backlogTask.priorityId)
          .map((e) => e.icon);
    ...
    },
    {
      ...
      render: (rowData) => {
        if (
          rowData.assignmentFullInfo == null ||
  ...
  async function getSuggestion() {
    ...
    selectedRows.forEach((taskId) => {
      if (
        taskList.find((e) => e.backlogTask.backlogTaskId === taskId)
          .assignableFullInfo.length > 0
      ) {
        body.push(taskId);
      } else {
    ...
    result.forEach((assignment) => {
      let duration = calcDuration(
        assignment.backlogTask.dueDate,
        assignment.backlogTask.fromDate
      );
    ...
    unassignableTasks.forEach((taskId) => {
      tableResultData.push({
        taskId: taskId,
        taskName: taskList.find((e) => e.backlogTask.backlogTaskId === taskId)
          .backlogTask.backlogTaskName,
        assign: null,
        duration: 0,
        assignable: [],
      });
    ...
    if (datasets[0].backgroundColor.length < labels.length) {
      for (
        let i = 0;
        i < labels.length / datasets[0].backgroundColor.length + 1;
        i++
      ) {
  ...
  function handleOnSelectionChange(rows) {
    const displayingIds = currentTableData.map(
      (e) => e.backlogTask.backlogTaskId
    );
    let selectedRowsNotDisplayingIds = selectedRows.filter(
      (row) => !displayingIds.includes(row)
    );
    setSelectedRows([
      ...selectedRowsNotDisplayingIds,
      ...rows.map((e) => e.backlogTask.backlogTaskId),
    ]);

    let selectedRowsNotDisplaying = taskList.filter(
      (row) => !displayingIds.includes(row.backlogTask.backlogTaskId)
    );
    ...
    let body = (
      <div style={modalStyle} className={classes.paper} onClick={handleClose}>
        ...
        >
          <Grid item xs="auto">
            <img
              src={API_URL + "/resources/backlog/" + item}
              style={{
    ...
    return (
      <div>
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={4} className={classes.grid}>
                  <Tooltip title="Biểu đồ dự án">
                    <IconButton
                      aria-label="projectChart"
                      onClick={() => {
                        history.push("/backlog/dashboard/" + backlogProjectId);
                      }}
                    >
                  ...
                  <Tooltip title="Danh sách thành viên">
                    <IconButton
                      aria-label="memberList"
                      onClick={(event) => {
                        setShowMemberListDialogOpen(true);
                      }}
                    >
                    ...
                    <IconButton
                      ...
                      onClick={(event) => {
                        ...
                        if (!isTableSelectable)
                          setOpenSuggestInstructionAlert(true);
                        setIsTableSelectable(!isTableSelectable);
                      }}
                    >
                      <HiLightBulb
                        color={isTableSelectable ? "#ff9100" : "#3F51B5"}
                        size="1.5em"
                      />
                    </IconButton>
                  </Tooltip>
              ...
              data={(query) => {
                return new Promise((resolve, reject) => {
                  ...
                  if (query.orderBy !== undefined) {
                    sortParam =
                      "&sort=" +
                      convertSortingField(query.orderBy.field) +
                      "," +
                  ...
                  if (query.filters.length > 0) {
                    ...
                    filter.forEach((v) => {
                      filterParam +=
                        convertFilterField(v.column.field) +
                        "=" +
                        v.value +
                        "&";
                    });
                  ...
                  if (!isShowMyTask)
                    api = "/backlog/get-project-detail-by-page/";
                    ...
                    return (
                      <Box my={2.5} mx={7.5}>
                        ...
                        <TextField
                          ...
                          variant="outlined"
                          value={
                            rowData.backlogTask.backlogDescription === null ||
                            rowData.backlogTask.backlogDescription === undefined
                              ? ""
                              : rowData.backlogTask.backlogDescription
                          }
                          multiline={true}
                          rows="5"
                          fullWidth
                          ...
                          <Box mt={1}>
                            <FormControl>
                              ...
                              <Select
                                ...
                                onChange={(event) => {
                                  handleUpdateTaskStatus(event, rowData);
                                }}
                                style={{ width: "200px" }}
                              >
                                {statusPool.map((item) => (
              ...
              actions={[
                {
                  ...
                  icon: () => {
                    return (
                      <ListAltIcon
                        color={isShowMyTask ? "primary" : "disabled"}
                        fontSize="large"
                      />
                    );
                  },
                ...
                {
                  icon: () => {
                    return <AddBoxIcon color="primary" fontSize="large" />;
                  },
                  tooltip: "Thêm task",
                  isFreeAction: true,
                  onClick: (event) =>
                ...
                (rowData) => ({
                  icon: () => {
                    return (
                      <EditIcon
                        color={rowData.editable ? "primary" : "disabled"}
                      />
                    );
                  },
                  tooltip: "Chỉnh sửa",
        ...
        <AlertDialog
          ...
          onClose={() => {
            setOpenSuggestInstructionAlert(false);
          }}
          severity="info"
        ...
        >
          <Alert
            onClose={handleCloseUpdateStatusAlert}
            severity={updateStatusAlert.severity}
            variant="filled"
          >
