...
export function InvoiceDetail() {
  ...
  const invoiceItemColumns = [
    ...
    { title: "Thành tiền (VND)", field: "amount" },
    {
      title: "Mã đơn hàng",
      render: (rowData) => (
        <Link to={"/orders/detail/" + rowData["orderId"]}>
          {rowData["orderId"]}
        </Link>
      ),
    },
  ...
  const paymentApplicationColumns = [
    {
      title: "Mã thanh toán",
      render: (rowData) => (
        <Link to={"/payment-group/payment-application/" + rowData["paymentId"]}>
          {rowData["paymentId"]}
        </Link>
      ),
    },
  ...
  const [paymentApplicationDataTable, setPaymentApplicationDataTable] =
    useState([]);
    ...
    let [invoice, invoiceItemDataTable, paymentApplicationDataTable] =
      await Promise.all([
        ...
        authGet(
          dispatch,
          token,
          "/get-all-invoice-item-by-invoice-id/" + invoiceId
        ),
        authGet(
          dispatch,
          token,
          "/get-all-payment-application-by-invoice-id/" + invoiceId
        ),
      ]);
  ...
  return (
    <div>
      <InvoiceDataTable
        ...
        actions={
          invoiceId && invoice["amount"] - invoice["paidAmount"] > 0
            ? [
                <Button
                  color={"primary"}
                  variant={"contained"}
                  onClick={() =>
                    history.push(
                      "/payment-group/quick-create-payment-application/" +
                        invoiceId
                    )
                  }
                >
  ...
  return (
    <div>
      <Button
        color="primary"
        variant="contained"
        onClick={() => history.push("/payment-group/create-payment")}
      >
        Thêm mới
      </Button>
      <InvoiceDataTable
  ...
  async function getDataTable() {
    let [payment, paymentApplications] = await Promise.all([
      ...
      authGet(
        dispatch,
        token,
        "/get-payment-application-by-payment-id/" + paymentId
      ),
    ]);
  ...
  return (
    <div>
      <Button
        ...
        variant="contained"
        onClick={() =>
          history.push("/payment-group/create-payment-application/" + paymentId)
        }
      >
        Thêm mới
      </Button>
...
export function DistributorUnpaidInvoiceList() {
  ...
  const columns = [
    {
      ...
      render: (rowData) => (
        <Link to={"/distributor-unpaid-invoice-detail/" + rowData["partyId"]}>
  ...
  async function getDataTable() {
    let response = await authGet(
      dispatch,
      token,
      "/get-unpaid-invoices-group-by-distributor-id"
    );
    setDataTable(response);
  }
...
export function DistributorUnpaidInvoiceDetail() {
  ...
  const columns = [
    ...
    {
      title: "Tổng nợ",
      render: (rowData) => rowData["amount"] - rowData["paidAmount"],
    },
  ];
  ...
  async function getDataTable() {
    let response = await authGet(
      dispatch,
      token,
      "/get-unpaid-invoice-by-distributor-id/" + partyDistributorId
    );
    setDataTable(response);
