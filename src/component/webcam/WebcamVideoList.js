...
const COLUMN_DISPLAY = ['userLoginId', 'createdDate', 'contentId', 'popupVideo'];
const COLUMN_RENAME = {
...
export function WebcamVideoList() {
  ...
  const [columnRender,] = useState(({
    ...
    'popupVideo': rowData => (<Button color={"primary"} variant={"contained"}
                                      onClick={e => openPopupVideo(rowData['objectId'])}>Play</Button>),
  }));
  ...
  async function getWebcamVideos() {
    ...
    if (webcamVideoKeys.length > 0) {
      ...
      setColumns(COLUMN_DISPLAY.map(key => ({
        field: key,
  ...
  return (<div>
    ...
    <Dialog open={videoPopupOpen} onClose={() => setVideoPopupOpen(false)} maxWidth={"xl"}>
      <DialogContent>
