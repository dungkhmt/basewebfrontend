...
export default function Approve() {
  ...
  async function getUserRegisterList() {
    let userRegisterList = await authGet(dispatch, token, '/user/get-all-register-user');
  ...
  async function handleApprove(userLoginId) {
    ...
    let response = await authGet(dispatch, token, '/user/approve-register/' + userLoginId);
  ...
  return (<div>
    <MaterialTable title={'Danh sách người dùng cần phê duyệt'}
                   columns={columns}
