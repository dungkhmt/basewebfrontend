...
export default function SolverConfigParam() {
  ...
  const [
    ...
    setMaxDistanceConsecutiveLocationTripAllowed,
  ] = useState(0);
  const [maxLocationsPerTripAllowed, setMaxLocationsPerTripAllowed] =
    useState(0);
  ...
  async function getData() {
    let solverConfigParam = await authGet(
      dispatch,
      token,
      "/get-solver-config-param"
    );

    setMaxTripDistance(solverConfigParam["maxTripDistance"]);
    setMaxDistanceConsecutiveLocationTripAllowed(
      solverConfigParam["maxDistanceConsecutiveLocationTripAllowed"]
    );
    setMaxLocationsPerTripAllowed(
      solverConfigParam["maxLocationsPerTripAllowed"]
    );
        ...
        <Card>
          <CardContent>
            ...
            {textFieldNumberFormat(
              ...
              maxTripDistance,
              (newValue) =>
                notNegativeFilterOnChange(newValue, setMaxTripDistance)
            )}
            {textFieldNumberFormat(
              "maxDistanceConsecutiveLocationTripAllowed",
              "maxDistanceConsecutiveLocationTripAllowed",
            ...
            {textFieldNumberFormat(
              ...
              fixLoadTime,
              (newValue) =>
                notNegativeIntFilterOnChange(newValue, setFixLoadTime)
            )}
            {textFieldNumberFormat(
              "fixUnloadTime",
              "fixUnloadTime",
              fixUnloadTime,
              (newValue) =>
                notNegativeIntFilterOnChange(newValue, setFixUnloadTime)
            )}
            {textFieldNumberFormat(
              "loadRate",
              "loadRate",
            ...
            {textFieldNumberFormat(
              ...
              manualUnloadTime,
              (newValue) =>
                notNegativeFilterOnChange(newValue, setManualUnloadTime)
            )}
            {textFieldNumberFormat(
              "palletUnloadTime",
              "palletUnloadTime",
              palletUnloadTime,
              (newValue) =>
                notNegativeFilterOnChange(newValue, setPalletUnloadTime)
            )}
          </CardContent>
          <CardActions>
            <Button
