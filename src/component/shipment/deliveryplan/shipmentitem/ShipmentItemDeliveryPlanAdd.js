...
export default function ShipmentItemDeliveryPlanAdd() {
  ...
  function handleSelectRow(rows) {
    rowSelectedMap[pageNumber] = new Set(
      rows.map((row) => row["shipmentItemId"])
    );
    // console.log(rows);
  ...
  function handleSubmit() {
    let shipmentItemDeliveryPlan = {
      deliveryPlanId,
      shipmentItemIds: Object.values(rowSelectedMap).flatMap((value) => [
        ...value,
      ]),
    };
    authPost(
      dispatch,
      token,
      "/create-shipment-item-delivery-plan",
      shipmentItemDeliveryPlan
    )
      .then((response) => {
        if (response.ok) {
          alert(
            "Update successful " +
              shipmentItemDeliveryPlan["shipmentItemIds"].length +
              " items"
          );
          history.push(
            process.env.PUBLIC_URL +
              "/shipment-item-delivery-plan/" +
              deliveryPlanId +
              "/list"
          );
        } else {
  ...
  async function getDataTable() {
    ...
    let response = await authGet(
      dispatch,
      token,
      "/shipment-item-of-user-login-not-in-delivery-plan/" +
        deliveryPlanId +
        "/all"
    );
    setWaiting(false);
