...
export default function ShipmentItemDeliveryTripDetailList() {
  ...
  async function getDataTable() {
    setWaiting(true);
    let shipmentItemInfo = await authGet(
      dispatch,
      token,
      "/shipment-item-info/" + shipmentItemId
    );
    setWaiting(false);
  ...
  return (
    <div>
      ...
      <Grid container spacing={3}>
        ...
        >
          <b>Mã: </b> {info["shipmentItemId"] ? info["shipmentItemId"] : ""}{" "}
          <p />
        ...
        >
          <div style={{ textAlign: "left" }}>
            <b>Mã sản phẩm: </b> {info["productId"] ? info["productId"] : ""}{" "}
            <p />
            <b>Tên sản phẩm: </b>{" "}
            {info["productName"] ? info["productName"] : ""} <p />
      ...
      {waiting ? (
        ...
        <MaterialTable
          title={"Danh sách chuyến giao hàng được gán"}
          columns={columns}
          data={dataTable}
