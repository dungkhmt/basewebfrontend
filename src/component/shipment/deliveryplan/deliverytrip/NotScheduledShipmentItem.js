...
export default function NotScheduledShipmentItem() {
  ...
  function getFacilityList() {
    authPost(dispatch, token, "/get-list-facility", {})
      .then((r) => r.json())
      .then((response) => {
        ...
        setDataTable(
          shipmentItemData.filter(
            (shipmentItem) =>
              shipmentItem["facilityId"] === facilities[0]["facilityId"]
          )
        );
      })
      .catch(console.log);
  ...
  async function handleSubmit(deliveryTripId) {
    ...
    let body = selectedRows.map((shipmentItem) => ({
      ...
      deliveryQuantity:
        shipmentItem["quantity"] - shipmentItem["scheduledQuantity"],
    }));
    setWaitData(true);
    let numberAddedShipmentItems = await authPost(
      dispatch,
      token,
      "/create-delivery-trip-detail/" + deliveryTripId,
      body
    ).then((r) => r.json());
    setWaitData(false);
  ...
  async function getShipmentItemData() {
    setWaitData(true);
    let shipmentItems = await authGet(
      dispatch,
      token,
      "/shipment-item-not-scheduled/" + deliveryPlanId
    );
    setWaitData(false);
    ...
    setDataTable(
      shipmentItems.filter(
        (shipmentItem) => shipmentItem["facilityId"] === selectedFacility
      )
    );
  }
  ...
  async function handleSuggest() {
    ...
    let response = await authPost(dispatch, token, "/suggest-trips", body).then(
      (r) => r.json()
    );
    ...
    <div>
      ...
      {waitData ? (
        ...
        <div>
          ...
          <Select
            ...
            onChange={(event) => {
              ...
              setDataTable(
                shipmentItemData.filter(
                  (shipmentItem) =>
                    shipmentItem["facilityId"] === event.target.value
                )
              );
            }}
          >
          ...
          <MaterialTable
            title={"Danh sách các đơn hàng chưa được xếp chuyến"}
            columns={columns}
            data={dataTable}
...
function TripSuggestion(props) {
  ...
  const columns = [
    {
      title: "Mã chuyến",
      render: (rowData) => rowData["deliveryTripModel"]["deliveryTripId"],
    },
    {
      title: "Tổng khối lượng",
      render: (rowData) =>
        round(rowData["deliveryTripModel"]["totalWeight"], 2) +
        " (+" +
    ...
    },
    {
      ...
      render: (rowData) =>
        toFormattedTime(rowData["deliveryTripModel"]["totalExecutionTime"]) +
        " (+" +
    ...
    },
    {
      title: "Quãng đường",
      render: (rowData) =>
        Math.round(rowData["deliveryTripModel"]["totalDistance"]) +
        " (+" +
    ...
    {
      title: "Mã xe",
      render: (rowData) => rowData["deliveryTripModel"]["vehicleId"],
    },
    {
      title: "Loại xe",
      render: (rowData) =>
        rowData["deliveryTripModel"]["vehicleProductTransportCategoryId"],
    },
    {
      title: "",
      render: (rowData) => (
        <Button
          variant={"contained"}
          onClick={() =>
            handleSubmit(rowData["deliveryTripModel"]["deliveryTripId"])
          }
        >
          Thêm vào chuyến này
        </Button>
  ...
  return (
    <div>
      ...
      >
        ...
        <MaterialTable
          title={"Chi tiết các chuyến được gợi ý"}
          columns={columns}
          data={trips}
