...
export default function DeliveryTripCreate() {
  ...
  function getVehicleList() {
    authGet(dispatch, token, "/vehicle/" + deliveryPlanId + "/all")
      .then((response) => {
  ...
  const handleSubmit = () => {
    ...
    authPost(dispatch, token, "/create-delivery-trip", deliveryTripInfo).then(
      (response) => {
        ...
        history.push(
          process.env.PUBLIC_URL + "/delivery-plan/" + deliveryPlanId
        );
      },
              ...
              >
                {vehicleList.map((vehicle) => (
                  <MenuItem value={vehicle["vehicleId"]}>
                    {vehicle["vehicleId"] + " (" + vehicle["capacity"] + " kg)"}
                  </MenuItem>
                ))}
              </Select>
              <p />
