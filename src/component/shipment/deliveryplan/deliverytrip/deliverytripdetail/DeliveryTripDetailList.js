...
export default function DeliveryTripDetailList() {
  ...
  function handleDelete(deliveryTripDetailId) {
    authGet(
      dispatch,
      token,
      "/delete-delivery-trip-detail/" + deliveryTripDetailId
    )
      .then((response) => {
        if (response) {
  ...
  const columns = [
    ...
    { title: "Địa chỉ", field: "address" },
    {
      title: "Tọa độ",
      field: "latLng",
      render: (rowData) => rowData["lat"] + "," + rowData["lng"],
    },
    { title: "Mã sản phẩm", field: "productId" },
    { title: "Tên sản phẩm", field: "productName" },
    {
      title: "Tổng số lượng sản phẩm trong đơn hàng",
  ...
  const NOT_EDITABLE_TRIP_STATUS = new Set([
    "DELIVERY_TRIP_APPROVED_TRIP",
  ...
  const getDeliveryTripInfo = () => {
    authGet(
      dispatch,
      token,
      "/delivery-trip/" + deliveryTripId + "/basic-info"
    ).then((response) => {
  ...
  function getDataTable() {
    authGet(dispatch, token, "/delivery-trip-detail/" + deliveryTripId)
      .then((response) => {
        console.log(response);
        setDataTable(response["orderItems"]);
        setFacilityLatLng({
          lat: response["depotLat"],
          lng: response["depotLng"],
        });
      })
      .catch(console.log);
  }
  ...
  async function getDriverInfo() {
    Promise.all([
      authGet(dispatch, token, "/get-all-drivers"),
      authGet(
        dispatch,
        token,
        "/get-driver-in-delivery-trip/" + deliveryTripId
      ),
  ...
  function handleUpdateDriver(driverId) {
    authGet(
      dispatch,
      token,
      "/set-driver-to-delivery-trip/" + deliveryTripId + "/" + driverId
    )
      .then((response) => {
        console.log(response);
      })
  ...
  function getLoadedRate() {
    if (deliveryTrip) {
      return (
        Math.round(
          (10000.0 * deliveryTrip["totalWeight"]) /
            deliveryTrip["maxVehicleCapacity"]
        ) / 100.0
      );
    }
    return 0;
          ...
          Toolbar: (props) => (
            <div>
              ...
              <Grid container spacing={3}>
                ...
                >
                  <b>Mã chuyến hàng: </b>{" "}
                  {deliveryTrip === null ? "" : deliveryTrip["deliveryTripId"]}{" "}
                  <p />
                  <b>Mã đợt giao hàng: </b>{" "}
                  {deliveryTrip === null ? (
                    ""
                  ) : (
                    <Link
                      to={`/delivery-plan/${deliveryTrip["deliveryPlanId"]}`}
                    >
                      {deliveryTrip["deliveryPlanId"]}
                    </Link>
                  )}
                  <p />
                  <b>Ngày tạo: </b>{" "}
                  {deliveryTrip === null ? "" : deliveryTrip["executeDate"]}{" "}
                  <p />
                  <b>Xe: </b>{" "}
                  {deliveryTrip === null ? "" : deliveryTrip["vehicleId"]}
                  <p />
                  <b>Tải trọng: </b>{" "}
                  {deliveryTrip === null
                    ? ""
                  ...
                  <p />
                  <b>Loại xe: </b>{" "}
                  {deliveryTrip === null ? "" : deliveryTrip["vehicleTypeId"]}
                  <p />
                  <b>Tài xế: </b>
                  <Select
                ...
                >
                  <div style={{ textAlign: "left" }}>
                    <b>Trạng thái chuyến: </b>{" "}
                    {deliveryTrip == null
                      ? 0
                      : STATUS_VN_TRANS[deliveryTrip["statusId"]]}{" "}
                    <p />
                    <b>Tổng khoảng cách: </b>{" "}
                    {deliveryTrip == null ? 0 : deliveryTrip["totalDistance"]}{" "}
                    <p />
                    <b>Tổng khối lượng (kg): </b>{" "}
                    {deliveryTrip == null
                      ? "0 (0 %)"
                      : `${
                          Math.round(deliveryTrip["totalWeight"] * 100) / 100.0
                        } (${getLoadedRate()} %)`}{" "}
                    <p />
                    <b>Tổng số pallet: </b>{" "}
                    {deliveryTrip == null ? 0 : deliveryTrip["totalPallet"]}{" "}
                    <p />
                  </div>
                  {deliveryTrip && deliveryTrip["editable"] ? (
                    <Link
                      to={
                        "/delivery-group/create-delivery-trip-detail/" +
                        deliveryTripId
                      }
                    >
                      <Button
                    ...
                    <Button
                      ...
                      onClick={() => {
                        authGet(
                          dispatch,
                          token,
                          "/approve-delivery-trip/" + deliveryTripId
                        ).then((r) => r);
                        window.location.reload();
                      }}
                    >
              ...
              routes={[
                {
                  post: facilityLatLng
                    ? {
                        lat: facilityLatLng["lat"],
                        lng: facilityLatLng["lng"],
                      }
                    : {},
                  items: dataTable[0]
