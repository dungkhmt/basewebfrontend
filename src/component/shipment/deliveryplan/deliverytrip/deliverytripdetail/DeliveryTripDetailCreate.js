...
export default function DeliveryTripDetailCreate() {
  ...
  const columns = [
    ...
    { title: "Tọa độ", field: "latLng" },
    {
      ...
      render: (rowData) => (
        <TextField
          ...
          margin="normal"
          inputProps={
            selectedIdSet.has(rowData["shipmentItemId"])
              ? {
                  min: "1",
    ...
    },
    {
      ...
      render: (rowData) =>
        Math.round(
          rowData["weight"] *
            selectedQuantity[rowData["shipmentItemId"]] *
            100 || 0
        ) / 100.0,
    },
  ];
      ...
      .then(
        (response) => {
          if (typeof response === "number") {
            showAlert(
              "Thành công",
              "Đã thêm vào chuyến cho " + response + " đơn hàng",
              {
                OK: () =>
                  history.push(
                    process.env.PUBLIC_URL +
                      "/delivery-group/delivery-trip/" +
                      deliveryTripId
                  ),
              }
            );
  ...
  function getDeliveryTripCapacityInfo(selectedRows, selectedQuantity) {
    ...
    authPost(
      dispatch,
      token,
      "/delivery-trip/" + deliveryTripId + "/capacity-info",
      body
    )
      .then((response) => response.json())
      .then((response) => {
            ...
            <Toolbar>
              <Grid container spacing={3}>
                <Grid
                  item
                  xs={7}
                  style={{ textAlign: "left", padding: "30px 30px 20px 10px" }}
                >
                  <div>
                ...
                >
                  <div>
                    <b>Tổng khoảng cách dự tính: </b>{" "}
                    {tripCapacityInfo == null
                      ? 0
                      : tripCapacityInfo["totalDistance"]}
                    <p />
                    <b>Tổng khối lượng (kg): </b>{" "}
                    {tripCapacityInfo == null
                      ? 0
                      : Math.round(tripCapacityInfo["totalWeight"] * 100) /
                        100.0}{" "}
                    <p />
                    <b>Tổng số pallet: </b>{" "}
                    {tripCapacityInfo == null
                      ? 0
                      : tripCapacityInfo["totalPallet"]}{" "}
                    <p />
                  </div>
                </Grid>
              </Grid>
