  ...
  const columns = [
    {
      ...
      render: (rowData) => (
        <Link to={"/delivery-plan/" + rowData["deliveryPlanId"]}>
          {rowData["deliveryPlanId"]}
        </Link>
      ),
    },
    ...
    {
      title: "Tổng khối lượng đã xếp chuyến (kg)",
      field: "totalWeightScheduled",
    },
  ];
        ...
        data={(query) =>
          new Promise(async (resolve) => {
            ...
            let [totalWeightShipmentItems, deliveryTrips] = await Promise.all([
              Promise.all(
                response.content.map((deliveryPlan) =>
                  authGet(
                    dispatch,
                    token,
                    "/get-total-weight-shipment-items-in-delivery-plan/" +
              ...
              Promise.all(
                response.content.map((deliveryPlan) =>
                  authGet(
                    dispatch,
                    token,
                    "/delivery-trip/" + deliveryPlan["deliveryPlanId"] + "/all"
                  )
                )
              ),
            ]);
            ...
            for (let i = 0; i < response.content.length; i++) {
              ...
              deliveryTrips[i].forEach(
                (deliveryTrip) =>
                  (totalWeightScheduled += deliveryTrip["totalWeight"])
              );

              totalWeight = Math.round(totalWeight * 100) / 100.0;
              totalWeightScheduled =
                Math.round(totalWeightScheduled * 100) / 100.0;
