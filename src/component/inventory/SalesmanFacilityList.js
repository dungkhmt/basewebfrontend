...
export function SalesmanFacilityList() {
    ...
    { field: "userLoginId", title: "UserLogin" },
    {
      ...
      render: (rowData) => (
        <Button
          color="secondary"
          variant="contained"
          onClick={() => handleDeleteFacilityRole(rowData["facilityRoleId"])}
        >
          Xóa
        </Button>
      ),
  ...
  async function getFacilityRoles() {
    let facilityRoles = await authGet(
      dispatch,
      token,
      "/get-all-facility-role?facilityId=" + facilityId
    );
  ...
  async function handleDeleteFacilityRole(facilityRoleId) {
    let response = await authGet(
      dispatch,
      token,
      "/delete-facility-role?facilityRoleId=" + facilityRoleId
    );
    if (!response) {
      alert("Có lỗi xảy ra");
    }
    ...
    <div>
      ...
      <table style={{ width: "100%", border: "0px" }}>
        ...
        <tr>
          <td>Địa chỉ:</td>
          <td>
            <b>{(facility["postalAddress"] || {})["address"]}</b>
          </td>
        </tr>
      </table>
      ...
      <div style={{ textAlign: "right" }}>
        <Button
          ...
          color={"primary"}
          onClick={() =>
            history.push("/facility/salesman/create/" + facilityId)
          }
        >
          Thêm mới
        </Button>
