...
export function SalesmanFacilityCreate() {
  ...
  async function getUserLoginIds(facilityRoleUserLoginIdSet) {
    let userLoginIds = await authGet(
      dispatch,
      token,
      "/get-all-user-login-ids"
    );
    if (userLoginIds) {
      userLoginIds = userLoginIds.filter(
        (userLoginId) => !facilityRoleUserLoginIdSet.has(userLoginId)
      );
  ...
  useEffect(() => {
    getFacilityRoles().then((facilityRoles) => {
      getUserLoginIds(
        new Set(facilityRoles.map((value) => value["userLoginId"]))
      ).then((r) => r);
    });
  }, []);
