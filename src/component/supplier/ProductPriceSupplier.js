...
export default function ProductPriceSupplier() {
  ...
  async function getProductPriceSupplierList() {
    let productPriceSupplier = await authGet(
      dispatch,
      token,
      "/get-all-product-price-supplier-by-supplier/" +
  ...
  async function getProductList() {
    let productList = await authPost(
      dispatch,
      token,
      "/get-list-product",
      {}
  ...
  async function handleSubmit() {
    let body = {
      supplierPartyId: selectedSupplier["partyId"],
      productId: selectedProduct["productId"],
      unitPrice,
    };
    let productPriceSupplier = await authPost(
      dispatch,
      token,
      "/set-product-price-supplier",
      body
    ).then((r) => r.json());
    if (
      productPriceSupplier &&
      productPriceSupplier["productPriceSupplierId"]
    ) {
      alert(
        "Đã thiết lập thành công giá, productPriceSupplierId = " +
          productPriceSupplier["productPriceSupplierId"]
      );
    }
