...
import { Input, TextField, MenuItem, Select, InputLabel, Card, Grid, Divider, 
    Button, IconButton, Tooltip, Fab, Box, Chip
        ...
        <Card style={{
            ...
            <form noValidate autoComplete="off">
                ...
                <Select
                    ...
                    fullWidth={true}
                    onOpen={(e)=>{setClicked(true);}}
                    onClose={(e)=>{if(selectedCourse == " ") setClicked(false); }}
                >
                    {
                        !clicked && 
                        <MenuItem value=" " key=" ">
                ...
                <Select
                    ...
                    fullWidth={true}
                    onOpen={(e)=>{setClicked2(true);}}
                    onClose={(e)=>{if(selectedClass == " ") setClicked2(false); }}
                >
                    {
                        !clicked2 && 
                        <MenuItem value=" " key=" ">
                            <em style={{color: "rgb(0,0,0,0.5)"}}>Bạn phải chọn học phần trước</em>
                        </MenuItem>
                    }
                    {listClassBelongCourse.map((option) => {
                        //console.log(option)
                        let displayStr = option.code + "     (" + option.classType + ")";
                ...
                <MuiPickersUtilsProvider utils={DateFnsUtils}>
                    ...
                    <Grid container spacing={10} >
                        ...
                        <Grid item xs={4}>
                            <TextField label="Tính bằng phút" placeholder="90" style={{marginTop: "15px"}} fullWidth
                                onChange={handleChangeDuration} value={duration} type="number"/>
                        </Grid>
                    </Grid>
                    </div>
            ...
            <div>
                <Tooltip title="Di chuyển xuống cuối trang" aria-label="Di chuyển xuống cuối trang">
                    <Fab color="primary" aria-label="down down" 
                        style={{
                            margin: 0,
                            top: 80,
                            right: 0,
                ...
                </Tooltip>
                {
                    questionList.map((elm, elmIndex) => {
                        return(
                            <div key={elmIndex}>
                                {elmIndex != 0 && 
                                    <div style={{ borderTop: "1px solid rgb(0, 0, 0, 0.5)", paddingBottom: '40px'}}></div>
                                }
                                <Grid container spacing={1} justify="center">
                                    ...
                                    <Grid item xs={11}>
                                        <div style={styles.descStyle}>
                                            ...
                                            <p style={{maxWidth: '1024px', overflowWrap: 'break-word'}}>
                                        ...
                                        <div style={styles.ansStyle}>
                                            {
                                                elm.ans.map((a, index) => {
                                                    return(
                                                        <div key={index + elmIndex} style={styles.subAnsStyle}>
                                                            
                                                            {a[1] == false ? 
                                                                <div style={{color: 'red'}}>{index + 1}. &#8287;{a[0]}</div> 
                                                                : <div style={{color: 'green'}}>{index + 1}. &#8287;{a[0]}</div>}
                                                        </div>
                                                    )
                                                })
                                            }
                                    ...
                                    <Grid item xs={1}>
                                        <div style={{}}>
                                            <Tooltip title="Di chuyển lên" aria-label="Di chuyển lên">
                                                <Button variant="outlined" color="primary" aria-label="Di chuyển lên" size='small' 
                                                    onClick={ (e) => { handleSwapQuestion(e, elmIndex, 'up') }}>
                                                    <KeyboardArrowUpIcon />
                                                </Button>
                                            </Tooltip>
                                            {nextLine}
                                            <Tooltip title="Di chuyển xuống" aria-label="Di chuyển xuống">
                                                <Button variant="outlined" color="primary" aria-label="Di chuyển xuống" size='small' 
                                                    onClick={ (e) => { handleSwapQuestion(e, elmIndex, 'down') }}>
                                                    <KeyboardArrowDown />
                                                </Button>
                                            </Tooltip>
                                            {nextLine}
                                            <Tooltip title="Xóa" aria-label="Xóa">
                                                <Button variant="outlined" color="secondary" aria-label="Xóa" size='small' 
                                                    onClick={(e) => { handleDelete(elmIndex); }}>
                                                    <DeleteIcon />
                                                </Button>
                                            </Tooltip>
                                        </div>
                ...
                <div style={{ borderTop: "1px solid rgb(0, 0, 0, 0.5)", paddingBottom: '35px'}}></div>
                ...
                <div style={{paddingLeft: '5%'}}>
                    <div style={{
                        ...
                        fontWeight: "lighter",
                        color: 'green'
                    }}>Đáp án đúng sẽ được in màu xanh </div>
                    {nextLine}
                    <div style={{
                        fontSize: "20px", 
                        fontWeight: "lighter",
                        color: 'red'
                    }}>Đáp án sai sẽ được in màu đỏ </div>
                </div>
                {lineBreak}
                <div style={{ borderTop: "1px solid rgb(0, 0, 0, 0.5)", paddingBottom: '35px'}}></div>
                {lineBreak}
                <Grid container spacing={1} justify="center">
                    <Grid item xs={12}>
                        <Button variant="outlined" style={{height: '100px', fontSize: '20px'}} fullWidth 
                            onClick={(e)=> { handleChangePage(0) }}>
