  ...
  dataPdf.map( (value,index) =>{
   ...
   studentResult.push(info)
    value['listQuestion'].map((ques,indexques) =>{
     ...
     let quesContent =htmlToPdfmake(ques["content"],{defaultStyles :{
      b: {bold:true,fontSize:16},
      strong: {bold:true,fontSize:16},
      u: {decoration:'underline',fontSize:16,
      bold:true},
      ...
      a: {color:'blue', decoration:'underline',bold:true,fontSize:16},
     ...
     ques["listAnswer"].map((ans,indAns)=>{
      let isChoose =  ques.listchooseAns.includes(ans.choiceAnswerId) ? true : false;
      if(!isChoose){
        console.log(value.fullName +"  "+ans.choiceAnswerContent )
      }
      let pele =  ans.isCorrectAnswer === 'Y' ? {
        margin:[0, 5, 0, 10],
        bold: isChoose ,
      ...
      let ansTmp =htmlToPdfmake(ans.choiceAnswerContent,{defaultStyles :{
        b: {bold:true},
        strong: {bold:true},
...
export function exportResultListPdf(studentListResult, resultExportPDFData, testId) {
  ...
  pdfMake.createPdf(docDefinitions).download('Danh sách kết quả test  ' + testId + '.pdf');
}
