...
function TeacherCourseQuizChoiceAnswerDetail(){
	...
	async function getQuizChoiceAnswerDetail(){
		await authGet(dispatch, token,'/get-quiz-choice-answer-detail/' + choiceAnswerId).then((res)=>{
			if (res){
    ...
    async function handleSubmit(){
        ...
        await authPost(dispatch, token, '/update-quiz-choice-answer/' + choiceAnswerId, body).then((res)=>{
			if (res.length !== 0 ){
	...
	async function handleDelete(){
		await authDelete(dispatch, token, '/delete-quiz-choice-answer/' + choiceAnswerId).then((res)=>{
			if (res.length !== 0 ){
    ...
    return(
        ...
        <MuiPickersUtilsProvider utils={DateFnsUtils}>
          ...
          <Button
            variant="contained"
            onClick={() => history.push("/edu/teacher/course/quiz/detail/" + questionId)}
          >
            Hủy
          </Button>
        </CardActions>
