  ...
  return <div>
    <TransportReport
      ...
      selectDataRequest={async () =>
        (await authPost(dispatch, token, '/get-list-facility', {}).then(r => r.json()))['facilities']}
...
export function TransportReportByPartyCustomer() {
  ...
  return <div>
    <TransportReport
      ...
      selectDataRequest={async () => await authGet(dispatch, token, '/get-list-party-customers')}
...
export function TransportReportByDriver() {
  ...
  return <div>
    <TransportReport
      ...
      filterName={'fullName'}
      selectDataRequest={async () => await authGet(dispatch, token, '/get-all-drivers')}
...
function TransportReport(props) {
  ...
  const {filterLabel, filterIdOnGetList, filterIdOnGetReports, filterName, selectDataRequest, tableTitle} = props;
  ...
  useEffect(() => {
    (async () => {
      ...
      await onChangeSelected(fromDate, thruDate, selectDataResponse[0][filterIdOnGetList]);
    })()
  }, []);

  async function onChangeSelected(fromDate, thruDate, selectedData) {
    let dataTableResponse = await authPost(dispatch, token, '/get-transport-reports', {
    ...
    <Grid container spacing={3}>
      <Grid item xs={8} style={{textAlign: 'left', padding: '0px 30px 20px 30px'}} justify="space-around">
        ...
        <Select value={selected} onChange={event => {
          ...
          onChangeSelected(fromDate, thruDate, event.target.value).then(r => r);
        }}>
          {selectData.map(value => <MenuItem value={value[filterIdOnGetList]}>
            {value[filterName] + ' (' + value[filterIdOnGetList] + ')'} </MenuItem>)}
        </Select>
      </Grid>
      <Grid item xs={4}
        ...
        <Button color={'primary'} variant={'contained'}> Xuất Excel </Button>
      </Grid>
    </Grid>

    <MaterialTable options={{search: false}} title={tableTitle} columns={columns} data={dataTable}/>
  </div>;
}