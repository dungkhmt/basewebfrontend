  ...
  return <div>
    <SaleReport
      ...
      selectDataRequest={async () =>
        (await authPost(dispatch, token, '/get-list-product', {}).then(r => r.json()))['products']}
...
function SaleReport(props) {
  ...
  const {filterLabel, filterId, selectDataRequest, tableTitle} = props;

  const columns = [
    {title: "Ngày", field: "orderDate",},
    {title: "Doanh thu (VND)", field: "totalGrand"},
  ];
  ...
  useEffect(async () => {
    ...
    await onChangeSelected(selectDataResponse[0][filterId]);
  }, []);

  async function onChangeSelected(newValue) {
    let dataTableResponse = await authPost(dispatch, token, '/get-sale-reports', {
      fromDate: toFormattedDateTime(fromDate), thruDate: toFormattedDateTime(thruDate), [filterId]: newValue // productId: ... || partyCustomerId: ...
    }).then(r => r.json());
    ...
    <Grid container spacing={3}>
      <Grid item xs={8} style={{textAlign: 'left', padding: '0px 30px 20px 30px'}} justify="space-around">
        ...
        <Select value={selected} onChange={event => {
          ...
          onChangeSelected(event.target.value).then(r => r);
        }}>
          {selectData.map(value => <MenuItem value={value[filterId]}> {value[filterId]} </MenuItem>)}
        </Select>
      </Grid>
      <Grid item xs={4}
