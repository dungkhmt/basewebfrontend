...
function SaleReport(props) {
  ...
  const {filterLabel, filterId, filterName, selectDataRequest, tableTitle} = props;
  ...
  async function onChangeSelected(fromDate, thruDate, selectedData) {
    let dataTableResponse = await authPost(dispatch, token, '/get-sale-reports', {
      fromDate: toFormattedDateTime(fromDate), thruDate: toFormattedDateTime(thruDate), [filterId]: selectedData // productId: ... || partyCustomerId: ...
    }).then(r => r.json());
    ...
    <Grid container spacing={3}>
      <Grid item xs={8} style={{textAlign: 'left', padding: '0px 30px 20px 30px'}} justify="space-around">
        ...
        <Select value={selected} onChange={event => {
          ...
          {selectData.map(value => <MenuItem value={value[filterId]}>
            {value[filterName] + ' (' + value[filterId] + ')'} </MenuItem>)}
        </Select>
      </Grid>
      <Grid item xs={4}
