...
const SOCKET_URL = 'http://localhost:8080/api/handlerMeet';
...
const Room = () => {
  ...
  const connect = () => {
    stompClient.connect({
      ...
      stompClient.subscribe('/topic/meet/' + roomId, (message) => handleMessage(JSON.parse(message.body)));
    });
  }
  ...
  sock.onclose = () => {
    stompClient.send("/app/meet/" + roomId, {}, JSON.stringify({name, type: 'leave'}));
  }
